document.addEventListener('DOMContentLoaded', () => {
  ...
  const drawingImageEl = document.getElementById('drawing-lightbox-image');
  if (drawingImages.length > 0 && drawingImageEl) {
    setupLightbox(drawingImages, drawingImageEl, 'drawing-lightbox-title');
  }

  // ========== FLASH ==========
  const flashImages = document.querySelectorAll('.flash-gallery img');
  const flashImageEl = document.getElementById('flash-lightbox-image');
  if (flashImages.length > 0 && flashImageEl) {
    flashImages.forEach((img, index) => {
      img.style.animationDelay = `${index * 0.3}s`;
    });
    setupLightbox(flashImages, flashImageEl, 'flash-lightbox-title');
  }

  // ========== PHOTOGRAPHY ==========
  const photoImages = document.querySelectorAll('.photography-gallery img');
  const photoImageEl = document.getElementById('photography-lightbox-image');
  if (photoImages.length > 0 && photoImageEl) {
    photoImages.forEach((img, index) => {
      img.style.animationDelay = `${index * 0.6}s`;
    });
    setupLightbox(photoImages, photoImageEl, 'photography-lightbox-title');
  }
